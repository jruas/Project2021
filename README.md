...
The full text file can be found in the project folder. All data are centimeters,
In terms of coding, there were a lot of steps before this one, but in terms of data analysis, this was the step where it was first possible to analyse the data, throughout the classic statistic parameters (number of samples, mininum and maximum values, average, standard deviation and percentils). The table obtained summarizes the data for the 4 measures: Speal Length, Sepal Width, Petal Length and Petal Width.
...
Hence, with an histogram, it is possible to analyse the distribution of the values[15].

First, an histogram for each variable was developed  (Sepal Lenght, Sepal With, Petal Lenght, Petal Width. This was obtained using the code below:

<img src="images/hist1.png" width="450">

The 4 plots were generated in the same image, for easier comparison. For that, the code had to be modified from a normal plot, defining the number of rows and columns in the generated plot, in this case 2 rows and 2 columns [16]. The plot generated is presented below and also saved in the project folder as generalHist.png:

<img src="generalHist.png" width="450">

From the histograms above, it is possible to realize that the Sepal Length varies from 4 to 8 (aprox), the Sepal Width from 2 to 4.5 (aprox), the Petal Length from 1 to 7 (aprox) and the Petal Width from 0 t0 2.5 (aprox). While Sepal Length and Sepal Width have a more "regular" histogram, Petal Lenght and Petal Width have almost like two patches of values. To understand what may be causing this, histograms for the same variables but showing the different kinds of species were developed:

<img src="4plots.png" width="450">

As it is possible to see, in Petal Lenght and Petal Width, the group of data on the left side of the histogram (meaning minor values), corresponds to only single species - Setosa, while the rest of the histogram has values from the other two species - Virginica and Versicolor. It is also possible to conclude that for the 4 variables, each species has its group of data together (in similar values). It is possible to see that species Setosa has the minimum values of the data for Petal Length, Sepal Length and Sepal Width, and maximum values of the data for Petal Width. It is also possible to see that the histogram for Virginica (in blue) is always a little to the right of histogram Versicolor (in red), which means that for the 4 parameters it has in general bigger values. Virginica has also the maximum values for Sepal Lenght, Petal Lenght and Petal Width. 
...
For this project, scatter plots where developed to analyse the correlation between all the variables, meaning that each variable was combined in a scatter with the 3 remaining variables. For each plot, the 3 species were identified, for a better perception of the results.

The code to get these plots was as in the following image: 
